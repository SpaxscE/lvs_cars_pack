...
function ENT:OnSpawn( PObj )
	...
	local DoorHandler = self:AddDoorHandler( "trunk", Vector(-55,0,13), Angle(0,0,0), Vector(-2,-20,-10), Vector(2,20,10), Vector(-30,-50,-10), Vector(2,-20,10) )
	...
	local FuelCap = self:AddDoorHandler( "fuel_cap", Vector(-41.55,-27.28,6.99), Angle(0,0,0), Vector(-3,-3,-3), Vector(3,3,3), Vector(-3,-3,-3), Vector(3,3,3) )
	FuelCap:SetSoundOpen( "lvs/vehicles/generic/car_door_open.wav" )
	FuelCap:SetSoundClose( "lvs/vehicles/generic/car_door_close.wav" )

	local FuelTank = self:AddFuelTank( Vector(-30,0,0), Angle(0,0,0), 1200, LVS.FUELTYPE_PETROL )
	FuelTank:SetDoorHandler( FuelCap )

	self:SetBodygroup( 1, 1 )


	self:AddEngine( Vector(47.12,0,15) )
